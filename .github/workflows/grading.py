...
def tokenBurn():
    for i in incomplete:
        call_function = web3.eth.contract(address=CONTRACT_ADDRESS, abi=ABI).functions.burn(STUDENT_ADDRESS,i,1).build_transaction({"chainId": web3.eth.chain_id, "from": OWNER_ADDRESS, "nonce": web3.eth.get_transaction_count(OWNER_ADDRESS)})
        signed_tx = web3.eth.account.sign_transaction(call_function, private_key=OWNER_PRIVATE_KEY)
        send_tx = web3.eth.send_raw_transaction(signed_tx.rawTransaction)
        tx_receipt = web3.eth.wait_for_transaction_receipt(send_tx)
        print(tx_receipt)
